...
**Phase 7 — closed 2026-08-20.** 7.a (HDR, tone mapping, colour grading), 7.b (Hosek-Wilkie procedural sky + equirectangular skybox), 7.c (image-based ambient: irradiance cube, prefiltered reflection cube, preintegrated BRDF table), 7.d (fog by distance and by height), 7.e (bloom) and 7.f (ambient occlusion and screen-space reflections) are built and verified on screen. Environment images decode on a worker thread; the sky rebuilds by comparing its inputs, and the ambient precompute waits for them to settle. The renderer is **five objects rather than one**: `DiligentRenderer` keeps the device, the scene targets, the pools and the scene pass, and `ShadowPass`, `EnvironmentMaps`, `ScreenSpaceEffects` and `PostChain` are members that each hold their own state in their own header. Bloom, fog, the IBL precompute, occlusion and reflections are all **ours, not DiligentFX's** — see the log for the prices that decided each. SSGI and SSIL were pruned in 7.f: parameter blocks with nothing behind them.

**Depth of field landed 2026-08-21**, after the phase closed rather than as part of it: a full-resolution, single-pass, CoC-weighted spiral gather in `PostChain`, running after bloom. See the log for why it needed a target of its own rather than reflections' same-texture trick, and the units gotcha its own test-scene default ran into.

**Next:** 8 (editor viewport), 9 (remove rlgl), 10 (skeletal animation), 11 (shader authoring).
...
**Remaining effort, order of magnitude (solo, part-time):** 7 is done · depth of field is a session or two whenever it is wanted · 8 only becomes work when the runtime moves off OpenGL · 9 <1 week · 10 is 2–4 months · 11 is 1–4 months, scope-dependent.
...
5. **Fog** — the only check that proves the depth reconstruction, and the one place a plausible-but-wrong answer hides. It is fully computable in closed form: drop the exposure until the floor is off 255, intersect the camera ray for a pixel with the floor plane, and predict that pixel. Nine points from 4.4 to 33.8 units matched within one unit per channel. The log has the arithmetic.

Occlusion and reflections add two exact checks of their own, and both are cheaper than looking:

- **With `ambient.energy` at zero, switching occlusion on must change nothing at all.** There is no ambient light to take away, so anything but zero pixels means the effect is reaching past the ambient target into direct light or emission. Measured: 0 of 3,504,640.
- **With `ssr.maxDistance` at zero, switching reflections on must change nothing at all.** No ray can travel, so every pixel takes the no-hit path — which proves the removal and the addition cancel exactly rather than approximately. Measured: 0 of 3,504,640.

Bloom adds a sixth, and it is the cheapest exact check in the file: **a flat region scales by exactly the intensity**. With one level and threshold zero, every region flat enough that the kernel cannot change the answer must come out `(1 + intensity)` times brighter in linear, so `1.25^(1/2.2) = 1.1068` encoded. Four more fall out of the same measurement — eight levels against one must be 1.0000 (spread, never brightness), a threshold above every scene value must be 1.0000, Mix at intensity 1 must be 1.0000 (the chain reproducing its own input), and Screen must never clip where Additive does. The log has all six measured.
...
- `rendering/diligent/` is **five classes across six translation units**, so a change lands in one of them and the others need not be opened. `DiligentRenderer` owns the device, the scene targets, the pools and the scene pass; `ShadowPass`, `EnvironmentMaps`, `ScreenSpaceEffects` and `PostChain` are its members, each declared in its own header beside the renderer's. `diligentInternal.h` holds only what more than one of them needs - the scene's cbuffer layouts and target formats, the ambient lookup both the scene pass and the reflection pass shade from, the cube-face table, the shadow-array capacities the scene shader is compiled against, and the two structs both geometry passes draw from. `diligentRenderer.cpp` is the shell (device, scene targets, frame boundary, raylib interop); then `diligentScene.cpp`, `diligentLighting.cpp` (the lights, and `ShadowPass` under them), `diligentEnvironment.cpp`, `diligentScreenSpace.cpp` and `diligentPost.cpp`. A constant that gains a second reader moves to `diligentInternal.h`; one that loses it moves back.
