...
**Before 7.f — decompose the renderer into sub-objects.** The five translation units are a mechanical split of one class; the class itself is still one object with every subsystem's pipelines, buffers and textures as its own members, which is why `diligentRenderer.h` is still 600 lines and is still the hardest part to read. The staged follow-up, chosen by the owner: `ShadowPass`, `EnvironmentMaps` and `PostChain` become members of `DiligentRenderer`, each with its own ~80-line header holding its own state, leaving the renderer's own header as the pools, the frame and the interop. Take them one at a time, `diff.ps1` to zero after each. The order that costs least is `PostChain` first (it touches only the scene target and its own targets), then `EnvironmentMaps`, then `ShadowPass` - which is last because the scene pass reads its arrays as static shader variables and the initialization order those need is an invariant.
...
- `rendering/diligent/` is **one class across five translation units**, so a change lands in one of them and the others need not be opened: `diligentRenderer.h` declares the whole class; `diligentInternal.h` holds only what more than one of them needs; `diligentRenderer.cpp` is the shell (device, scene target, frame boundary, raylib interop); then `diligentScene.cpp`, `diligentLighting.cpp`, `diligentEnvironment.cpp`, `diligentPost.cpp`. A constant that gains a second reader moves to `diligentInternal.h`; one that loses it moves back.
