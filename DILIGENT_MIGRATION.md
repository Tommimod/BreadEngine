...
**Phase 7 — in progress.** 7.a (HDR, tone mapping, colour grading), 7.b (Hosek-Wilkie procedural sky + equirectangular skybox), 7.c (image-based ambient: irradiance cube, prefiltered reflection cube, preintegrated BRDF table), 7.d (fog by distance and by height) and 7.e (bloom) are built and verified on screen. Environment images decode on a worker thread; the sky rebuilds by comparing its inputs, and the ambient precompute waits for them to settle. The renderer is **four objects rather than one** as of 2026-08-20: `DiligentRenderer` keeps the device, the scene target, the pools and the scene pass, and `ShadowPass`, `EnvironmentMaps` and `PostChain` are members that each hold their own state in their own header. Bloom is **ours, not DiligentFX's** — see the log for why `PostProcess/Bloom` was priced and turned down.

**Next:** 7.f the normal+roughness target with SSAO and SSR on it. Then 8 (editor viewport), 9 (remove rlgl), 10 (skeletal animation), 11 (shader authoring).

**What renders today:** all seven primitives and imported models, textured, normal-mapped, lit by up to 32 directional/spot/omni lights with range and cone falloff; cascaded shadows from one directional light, maps for up to four spots and cube maps for up to four omni lights; ambient from a sky through its own irradiance and reflection maps; fogged by distance, by height, or by both; bloomed by a thresholded mip chain in any of three blend modes; resolved through tone mapping and colour grading. In the editor viewport and in the standalone game alike. Still stubbed: SSAO, SSR, SSGI, SSIL and depth of field.
...
**7.f — the rest of the environment phase.** The parameter blocks in `configs/light/` are already engine-native and serialized, and `setEnvironment` is the single place that maps them onto DiligentFX's PostFX components — call sites should not change. **DiligentFX is adopted component by component** (3.5's decision, kept here so it is not re-argued): `PostProcess/` — DepthOfField, ScreenSpaceAmbientOcclusion, ScreenSpaceReflection, TemporalAntiAliasing — and `Components/ToneMapping.hpp` are what 7.f reaches for; `Components/ShadowMapManager.hpp` was Phase 6's. The IBL precompute was **ours to write** because `PrecomputeCubemaps` is a method on `PBR_Renderer`, and 7.c wrote it; so was fog, because DiligentFX has no fog component at all; so was bloom, for the reason below.

⚠️ **Price `PostFXContext` before reaching for any of those.** Every `PostProcess/` component is only reachable through it, and `PostFXContext::Execute` dereferences a previous-frame depth buffer and a motion-vector buffer unconditionally — neither of which the engine has — then runs four passes a frame whether the component reads them or not. That is what took `PostProcess/Bloom` off the table in 7.e; SSR is the effect that actually wants those inputs, so 7.f is where the cost is worth weighing rather than avoiding. 7.f's SSAO and SSR commit the scene pass to a second render target (normal+roughness). ⚠️ The append-only serialization rule is **lifted for `configs/light/` for the duration of Phase 7**, by the owner: blocks there may be restructured, and SSGI/SSIL may be pruned.
...
- `rendering/diligent/` is **four classes across five translation units**, so a change lands in one of them and the others need not be opened. `DiligentRenderer` owns the device, the scene target, the pools and the scene pass; `ShadowPass`, `EnvironmentMaps` and `PostChain` are its members, each declared in its own header beside the renderer's. `diligentInternal.h` holds only what more than one of them needs - the scene's cbuffer layouts, the cube-face table, the shadow-array capacities the scene shader is compiled against, and the two structs both geometry passes draw from. `diligentRenderer.cpp` is the shell (device, scene target, frame boundary, raylib interop); then `diligentScene.cpp`, `diligentLighting.cpp` (the lights, and `ShadowPass` under them), `diligentEnvironment.cpp` and `diligentPost.cpp`. A constant that gains a second reader moves to `diligentInternal.h`; one that loses it moves back.
