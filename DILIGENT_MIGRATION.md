...
1. **Phase 4** — its real remainder only: `MaterialHandle` over a per-material SRB, and refcounting in `TextureAsset`.
2. **Phase 5** — six primitive generators and the assimp importer.
...
3.6. **Remove R3D — DONE.** *(new after 3.c.)* Pulled forward from Phase 9 on the reasoning that less code is less to break, once the R3D screenshot diff had served its purpose. It was mechanical, as Phase 2's seam promised; the log has what was deleted and the two decisions taken along the way (the backend switch removed outright with the renderer moved up to `rendering/diligent/`, and `applyDefaultEnvironment` dropped in favour of engine-owned field initializers). `Slope`, `Torus` and `FreePoly` were dropped here too, ahead of Phase 5.
   ...
   ⚠️ **The capability cost was accepted deliberately and is still outstanding.** Deleting R3D could not break the Diligent build, but it did remove the only configuration in which the editor was *complete*. Models, nine of ten primitives, skybox and cubemaps, ambient/IBL maps, every post-effect, shadows and all lighting past one directional light are stubs, and that is now the only reality until Phases 5–7 land.
...
5. **Mesh primitives, MeshRenderer, SpriteRenderer.** Reimplement the remaining `R3D_GenMesh*` generators as engine-native geometry producing Diligent vertex/index buffers — genuinely new code (no Diligent equivalent exists), but standard parametric math, good AI-assist candidate.
   **`Slope`, `Torus` and `FreePoly` were dropped in 3.6**, so **six** generators are left — sphere, hemisphere, cylinder, capsule, plane, quad — on top of the cube 3.b already produced. The `forward` parameter on `createPrimitive`/`generatePrimitive` **stays**: `Quad` still needs it, even though `FreePoly` was its other user. A scene saved with one of the dropped types degrades rather than crashes — the switch in `MeshRenderer::deserializeMeshData` falls through and that node renders no mesh. Port `meshAsset`'s assimp import to build an engine-native cached mesh format directly (today assimp/R3D only transiently enumerate material count, geometry isn't cached) — this also lets Phase 9 scope assimp to import-time/editor-only, dropping it from the shipped game's runtime deps entirely.
