...
**Depth of field — the one effect still declared and stubbed.** `EnvironmentDOFParameters` describes a bokeh implementation (focus point, focus scale, near scale, max blur size) and nothing reads it. It belongs after bloom in the post chain, and it has everything it needs already: the scene depth is a shader resource, and `PostChain` is where a pass that finishes the image goes. Not a phase of its own — pick it up whenever the look calls for it.
